...
def fetch_readme(username):
    # Check if README is in the cache and is still valid
    ...
    try:
        ...
        # Cache the content and the current timestamp
        readme_cache[username] = {
...
def fetch_avatar(username):
    ...
    try:
        ...
        return user_data.get('avatar_url', 'default_avatar.png')  # Fallback to a default image if not found
    except requests.exceptions.RequestException:
        return 'default_avatar.png'  # Fallback to a default image if there's an error
